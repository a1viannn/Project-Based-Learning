    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown no-arrow">
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                ...
                <!-- jika user memiliki kedua peran -->
                {{-- @if (Auth::user()->hasRole(['anggota']) && Auth::user()->hasRole(['pengurus']))
                    <div class="dropdown-divider"></div>
                    @if (session('current_role') == 'anggota')
                        <a class="dropdown-item" href="{{ route('switch.to.pengurus') }}">
                            <i class="fas fa-exchange-alt fa-sm fa-fw mr-2 text-gray-400"></i>
                            Switch to Pengurus
                        </a>
                    @else
                        <a class="dropdown-item" href="{{ route('switch.to.anggota') }}">
                            <i class="fas fa-exchange-alt fa-sm fa-fw mr-2 text-gray-400"></i>
                            Switch to Anggota
                        </a>
                    @endif
